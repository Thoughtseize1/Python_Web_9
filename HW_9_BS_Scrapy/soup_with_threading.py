...
def grab_author_biography(session, page_url):
...
def collect_authors_on_page(session, page_url):
    ...
    for div in div_class_quote:
        ...
        (
            ...
            author["description"],
        ) = grab_author_biography(session, author_link)
...
if __name__ == "__main__":
    with requests.Session() as session:
        with Manager() as manager:
            ...
            def collect_authors_and_qoutes_on_page(page_url):
                response = session.get(page_url)
                soup = BeautifulSoup(response.text, "html.parser")
                div_class_quote = soup.find_all("div", class_="quote")
                for div in div_class_quote:
                    ...
                    quote = {}
                    quote["tags"] = [
                        tag.text for tag in div.find_all("a", class_="tag")
                    ]
                    quote["author"] = name_of_author
                    quote["quote"] = div.span.text
                    qoutes.append(quote)
            ...
            with Pool(processes=4) as pool:
                results = pool.imap(
                    collect_authors_and_qoutes_on_page,
                    collect_pages_urls(session, base_url),
                )
        ...
        write_to_json("authors", list(authors))
        write_to_json("qoutes", list(qoutes))
